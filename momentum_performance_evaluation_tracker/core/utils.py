...
def calculate_performance_score(employee):
    ...
    try:
        attendance = calculate_attendance_rate(employee)
        compliance = calculate_compliance_rate(employee)
        backlog_count = calculate_backlog_count(employee)

        # Convert backlog count to a score (fewer backlogs = higher score)
        backlog_score = max(0, 100 - (backlog_count * 5))

        # Weighted average
        overall_score = (attendance * 0.3) + (compliance * 0.5) + (backlog_score * 0.2)
